...
command_pub         = rospy.Publisher('/car_4/offboard/command'.format(car_name), AckermannDrive, queue_size = 1)
polygon_pub         = rospy.Publisher('/car_4/purepursuit_control/visualize'.format(car_name), PolygonStamped, queue_size = 1)
steering_marker_pub = rospy.Publisher('/car_4/steering_angle_marker', Marker, queue_size=1)
...
def construct_path():
    ...
    file_path = os.path.expanduser('~/catkin_ws/src/f1tenth_purepursuit/path/raceline_high_res.csv'.format(trajectory_name))
...
def purepursuit_control_node(data):
    ...
    # print(alpha)
    steering_angle = math.degrees(math.atan(2*WHEELBASE_LEN*math.sin(alpha)/lookahead_distance))
...
if __name__ == '__main__':
    ...
    try:
        ...
        rospy.Subscriber('/car_4/particle_filter/viz/inferred_pose'.format(car_name), PoseStamped, purepursuit_control_node)
        rospy.spin()
